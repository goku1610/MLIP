...
TEST("RB_vdW_damped reading and writing test") {
	
	double min_dist = 0.0;
	double max_dist = 6.0;
	int basis_function_count = 2;
	RadialBasis_vdW_damped bas(min_dist, max_dist, basis_function_count, -0.5, 30);

	std::string file = "../../dev_test/RB_damped_basis.txt";

	ofstream strm;
    strm.open(file, ios_base::out);
    if (!strm.is_open()) {
        cout << "File could not be opened!" << endl;
        FAIL()
    }    

	bas.WriteRadialBasis(strm);
	...
	RadialBasis_vdW_damped bas2(20, 50, 1);
	
	ifstream strm2;
    strm2.open(file, ios_base::in);
    if (!strm2.is_open()) {
        cout << "File could not be opened!" << endl;
        FAIL()
    }    

	bas2.ReadRadialBasis(strm2);
	...
	if (bas.a1 != bas2.a1 || bas.a2 != bas2.a2)
	{
		std::cout << bas.a1 << " " << bas2.a1 << " " << bas.a2 << " " << bas2.a2 << std::endl;
		FAIL()
	}

} END_TEST;

TEST("compare RB_Calc(r) and RB_Calc(r,i,j) for some bases") {
	...
	double r = 3.5;
        int i = 1;
        int j = 2;
	std::vector<double> rb_vals_to_compare(basis_function_count);
	std::vector<double> rb_ders_to_compare(basis_function_count);
        
	RadialBasis_Shapeev bas_shapeev(r_min, r_cut, basis_function_count);
	bas_shapeev.RB_Calc(r);
        for (int i = 0; i < basis_function_count; i++) {
            rb_vals_to_compare[i] = bas_shapeev.rb_vals[i];
            rb_ders_to_compare[i] = bas_shapeev.rb_ders[i];
        }
        bas_shapeev.RB_Calc(r,i,j);
        for (int i = 0; i < basis_function_count; i++) {
            if (rb_vals_to_compare[i] != bas_shapeev.rb_vals[i] && rb_ders_to_compare[i] != bas_shapeev.rb_ders[i]) {
                cout << rb_vals_to_compare[i] << " " << bas_shapeev.rb_vals[i] << " " << rb_ders_to_compare[i] << " " << bas_shapeev.rb_ders[i] << endl;
                FAIL()
            }
        }

	RadialBasis_Chebyshev bas_chebyshev(r_min, r_cut, basis_function_count);
	bas_chebyshev.RB_Calc(r);
        for (int i = 0; i < basis_function_count; i++) {
            rb_vals_to_compare[i] = bas_chebyshev.rb_vals[i];
            rb_ders_to_compare[i] = bas_chebyshev.rb_ders[i];
        }
        bas_chebyshev.RB_Calc(r,i,j);
        for (int i = 0; i < basis_function_count; i++) {
            if (rb_vals_to_compare[i] != bas_chebyshev.rb_vals[i] && rb_ders_to_compare[i] != bas_chebyshev.rb_ders[i]) {
                cout << rb_vals_to_compare[i] << " " << bas_chebyshev.rb_vals[i] << " " << rb_ders_to_compare[i] << " " << bas_chebyshev.rb_ders[i] << endl;
                FAIL()
            }
        }

	RadialBasis_Taylor bas_taylor(r_min, r_cut, basis_function_count);
	bas_taylor.RB_Calc(r);
        for (int i = 0; i < basis_function_count; i++) {
            rb_vals_to_compare[i] = bas_taylor.rb_vals[i];
            rb_ders_to_compare[i] = bas_taylor.rb_ders[i];
        }
        bas_taylor.RB_Calc(r,i,j);
        for (int i = 0; i < basis_function_count; i++) {
            if (rb_vals_to_compare[i] != bas_taylor.rb_vals[i] && rb_ders_to_compare[i] != bas_taylor.rb_ders[i]) {
                cout << rb_vals_to_compare[i] << " " << bas_taylor.rb_vals[i] << " " << rb_ders_to_compare[i] << " " << bas_taylor.rb_ders[i] << endl;
                FAIL()
            }
        }

	RadialBasis_Chebyshev_repuls bas_chebyshev_repuls(r_min, r_cut, basis_function_count);
	bas_chebyshev_repuls.RB_Calc(r);
        for (int i = 0; i < basis_function_count; i++) {
            rb_vals_to_compare[i] = bas_chebyshev_repuls.rb_vals[i];
            rb_ders_to_compare[i] = bas_chebyshev_repuls.rb_ders[i];
        }
        bas_chebyshev_repuls.RB_Calc(r,i,j);
        for (int i = 0; i < basis_function_count; i++) {
            if (rb_vals_to_compare[i] != bas_chebyshev_repuls.rb_vals[i] && rb_ders_to_compare[i] != bas_chebyshev_repuls.rb_ders[i]) {
                cout << rb_vals_to_compare[i] << " " << bas_chebyshev_repuls.rb_vals[i] << " " << rb_ders_to_compare[i] << " " << bas_chebyshev_repuls.rb_ders[i] << endl;
                FAIL()
            }
        }

	RadialBasis_vdW bas_vdW(r_min, r_cut, basis_function_count);
	bas_vdW.RB_Calc(r);
        for (int i = 0; i < basis_function_count; i++) {
            rb_vals_to_compare[i] = bas_vdW.rb_vals[i];
            rb_ders_to_compare[i] = bas_vdW.rb_ders[i];
        }
        bas_vdW.RB_Calc(r,i,j);
        for (int i = 0; i < basis_function_count; i++) {
            if (rb_vals_to_compare[i] != bas_vdW.rb_vals[i] && rb_ders_to_compare[i] != bas_vdW.rb_ders[i]) {
                cout << rb_vals_to_compare[i] << " " << bas_vdW.rb_vals[i] << " " << rb_ders_to_compare[i] << " " << bas_vdW.rb_ders[i] << endl;
                FAIL()
            }
        }

	CinfRadialBasis bas_cinf(r_min, r_cut, basis_function_count);
	bas_cinf.RB_Calc(r);
	for (int i = 0; i < basis_function_count; i++) {
		rb_vals_to_compare[i] = bas_vdW.rb_vals[i];
		rb_ders_to_compare[i] = bas_vdW.rb_ders[i];
	}
	bas_cinf.RB_Calc(r,i,j);
	for (int i = 0; i < basis_function_count; i++) {
		if (rb_vals_to_compare[i] != bas_vdW.rb_vals[i] && rb_ders_to_compare[i] != bas_vdW.rb_ders[i]) {
			cout << rb_vals_to_compare[i] << " " << bas_vdW.rb_vals[i] << " " << rb_ders_to_compare[i] << " " << bas_vdW.rb_ders[i] << endl;
			FAIL()
...
// TEST("Printing RB_vdW_damped functions") {
// 	double min_dist = 0.0;
// 	double max_dist = 6.0;
// 	int basis_function_count = 2;
// 	double r_min = 0.0;
// 	double r_max = 6.2;
// 	double step = 0.02;
// 	RadialBasis_vdW_damped bas(min_dist, max_dist, basis_function_count);

// 	fstream strm;
//     strm.open("../../../test/RB_damped_func.txt", ios_base::out);
//     if (!strm.is_open()) {
//         cout << "File could not be opened!" << endl;
//         FAIL()
//     }    
...
// 	double r = r_min;
// 	for (int i = 0; i <= floor((r_max-r_min) / step); i++)
// 	{
// 		strm << r;
// 		for (int k = 0; k < 2; k++)
// 			for (int l = k; l < 2; l++)
// 			{
// 				bas.RB_Calc(r, k, l);
// 				for (int j = 0; j < basis_function_count; j++)
// 					strm << " " << bas.rb_vals[j];
// 			}
// 		strm << endl;
// 		r += step;
...
TEST("check RB_vdW derivatives with finite differences") {
	double diffstep = 1e-4;
	int basis_function_count = 8;
	double r_min = 1.6;
	double r_cut = 5.2;
	double r = 3.5;
	RadialBasis_vdW bas(r_min, r_cut, basis_function_count);
	bas.RB_Calc(r);
	RadialBasis_vdW bas_p(r_min, r_cut, basis_function_count);
	bas_p.RB_Calc(r + diffstep);
	RadialBasis_vdW bas_m(r_min, r_cut, basis_function_count);
	bas_m.RB_Calc(r - diffstep);
	for (int i = 0; i < basis_function_count; i++) {
		double cdiff = (bas_p.rb_vals[i] - bas_m.rb_vals[i]) / (2 * diffstep);
		if (fabs((cdiff - bas.rb_ders[i])) > 2e-4) {
			std::cout << "\ni = " << i << ": " << fabs((cdiff - bas.rb_ders[i])) << " " << bas.rb_ders[i] << " " << cdiff << std::endl;
			FAIL()
		}
	}
} END_TEST;

TEST("check CinfRadialBasis derivatives with finite differences") {
        double diffstep = 1e-4;
        int basis_function_count = 8;
        double r_min = 1.6;
        double r_cut = 5.2;
        double r = 3.5;
        CinfRadialBasis bas(r_min, r_cut, basis_function_count);
        bas.RB_Calc(r);
        CinfRadialBasis bas_p(r_min, r_cut, basis_function_count);
        bas_p.RB_Calc(r + diffstep);
        CinfRadialBasis bas_m(r_min, r_cut, basis_function_count);
        bas_m.RB_Calc(r - diffstep);
        for (int i = 0; i < basis_function_count; i++) {
                double cdiff = (bas_p.rb_vals[i] - bas_m.rb_vals[i]) / (2 * diffstep);
                if (fabs((cdiff - bas.rb_ders[i])) > 2e-4) {
...
TEST("check CinfRadialBasis with random MTP parameters") {
	Configuration cfg;
	double displacement = 1e-5;
	double control_delta = 1e-3;

	string mtp_filename = PATH+"unlearned_cinf_2.mtp";
	...
	// p_mtpr->CalcEFS(cfg);
	if (!p_mtpr->CheckEFSConsistency_debug(cfg, displacement, control_delta))
		FAIL()

	ifs.close(); 
} END_TEST;

TEST("check RB_vdW with random MTP parameters") {
	Configuration cfg;
	double displacement = 1e-5;
	double control_delta = 1e-3;

	string mtp_filename = PATH+"unlearned_vdw_2.mtp";
	MLMTPR * p_mtpr = new MLMTPR(mtp_filename);
	vector<double> mtp_coeff(p_mtpr->CoeffCount());
...
TEST("check RB_vdW_damped derivatives with finite differences") {
	double diffstep = 1e-4;
	int basis_function_count = 2;
	double r_min = 1.6;
	double r_cut = 5.2;
	double r = 3.5;
	RadialBasis_vdW_damped bas(r_min, r_cut, basis_function_count);
	bas.RB_Calc(r, 0, 0);
	RadialBasis_vdW_damped bas_p(r_min, r_cut, basis_function_count);
	bas_p.RB_Calc(r + diffstep, 0, 0);
	RadialBasis_vdW_damped bas_m(r_min, r_cut, basis_function_count);
	bas_m.RB_Calc(r - diffstep, 0, 0);
	for (int i = 0; i < basis_function_count; i++) {
		double cdiff = (bas_p.rb_vals[i] - bas_m.rb_vals[i]) / (2 * diffstep);
		if (fabs((cdiff - bas.rb_ders[i])) > 2e-8) {
			std::cout << "\ni = " << i << ": " << fabs((cdiff - bas.rb_ders[i])) << " " << bas.rb_ders[i] << " " << cdiff << std::endl;
			FAIL()
		}
	}
...
TEST("check RB_vdW_damped with random MTP parameters") {
	Configuration cfg;
	double displacement = 1e-5;
	double control_delta = 1e-3;

	string mtp_filename = PATH+"unlearned_vdw_damped_2.mtp";
	MLMTPR * p_mtpr = new MLMTPR(mtp_filename);
	vector<double> mtp_coeff(p_mtpr->CoeffCount());
	
	for (int i = 0; i < p_mtpr->CoeffCount(); i++) {
		p_mtpr->Coeff()[i] = 3e-1*rand() / RAND_MAX;
		mtp_coeff[i] = p_mtpr->Coeff()[i];
	}
...
TEST("CalcE: CombinedAnyLocalMLIP vs pot1+pot2") {
	Configuration cfg;
	double control_delta = 1e-6;
	EAMMult eam(4, 4, 2, 1.6);

	for (int i = 0; i < eam.CoeffCount(); i++) {
		//eam.Coeff()[i] = 1.0 / pow(2, i);
		eam.Coeff()[i] = 0.15*sin(i + 1);
	}

	string mtp_filename = PATH+"unlearned_cheb_2.mtp";
	MLMTPR * p_mtpr = new MLMTPR(mtp_filename);
	vector<double> mtp_coeff(p_mtpr->CoeffCount());
	...
	CombinedAnyLocalMLIP combined_pot = CombinedAnyLocalMLIP(p_mtpr, &eam);
	...
	p_mtpr->CalcE(cfg);
	double en_mtpr = cfg.energy;

	eam.CalcE(cfg);
	double en_eam = cfg.energy;

	combined_pot.CalcE(cfg);
	double en_tot = cfg.energy;


	if (abs((en_tot-en_eam-en_mtpr) / en_tot) > control_delta)
	{
		cout << endl << en_tot << "  " << en_mtpr << "    " << en_eam << "    " << abs((en_tot-en_eam-en_mtpr) / en_tot) << endl;
		FAIL()
	}
...
TEST("CombinedAnyLocalMLIP (mtpr + eam) CalcEFS check with finite differences") {
	Configuration cfg;
	double displacement = 1e-5;
	double control_delta = 1e-3;
	...
	cfg.Load(ifs);
	if (!combined_pot.CheckEFSConsistency_debug(cfg, displacement, control_delta))
		FAIL()

	ifs.close(); 
} END_TEST;

TEST("CombinedAnyLocalMLIP (mtpr + mtpr) CalcEFS check with finite differences") {
	Configuration cfg;
	double displacement = 1e-5;
	double control_delta = 1e-3;
	...
	string mtp2_filename = PATH+"unlearned_vdw_2.mtp";
	MLMTPR * p_mtpr2 = new MLMTPR(mtp_filename);
	vector<double> mtp_coeff2(p_mtpr->CoeffCount());
	
	for (int i = 0; i < p_mtpr2->CoeffCount(); i++) {
	    p_mtpr2->Coeff()[i] = 3e-2*rand() / RAND_MAX;
	    mtp_coeff2[i] = p_mtpr2->Coeff()[i];
	}

	CombinedAnyLocalMLIP combined_pot = CombinedAnyLocalMLIP(p_mtpr, p_mtpr2);
