...
#include "../dev_src/combined_any_local_mlip.h"
...
//        ERROR("Improper MLIP type specified. (\"mtp\" or \"mtpr\" is required)");
    ...
    if (mlip_type == "combined") {

        std::stringstream add_fnms(additional_fnm);
        std::string curr_fnm;

        while(std::getline(add_fnms, curr_fnm, '+')) 
            p_mlip = new CombinedAnyLocalMLIP(p_mlip, new MLMTPR(curr_fnm));
	
        Message("Combined potential (mtpr+eam) instantiated");

        if (enable_learn || enable_select)
            ERROR("Incorrect settings. Combined mlip should be used with enable_learn = enable_select = False");
    }
